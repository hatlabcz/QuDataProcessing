...
def exponetialDecay_fit(xdata, ydata, CI=False, plot=True):
    ...
    mini = lmf.Minimizer(_residuals, fit_params, fcn_kws={'model':exponetialDecay_model, 'xdata': xdata, 'data': ydata})
    out = mini.leastsq()
    # out = lmf.minimize(_residuals, fit_params, method='powell', args=(exponetialDecay_model, xdata, data), nan_policy='omit')
    if plot:
        plt.figure()
        plt.plot(xdata, ydata, '*', label='data')
